...
public class ReverseListBox : ListBox
{
	...
	/* Initialize the box.
	 */
	public new void Initialize(ListPositionCtrl listPositionCtrl)
	{
	...
	private void InitialContent()
	{
		...
		//_contentID = (-_positionCtrl.listBoxes.Length / 2+_contentID) %_positionCtrl.listBoxes.Length;
		_contentID += _positionCtrl.listBoxes.Length / 2 - listBoxID;
	...
	private void UpdateToNextContent()
	{
		...
		if (_positionCtrl.listType == ListPositionCtrl.ListType.Linear)
		{
			if (_contentID == _listBank.GetListLength() - 1 ||
				!lastListBox.isActiveAndEnabled)
			{
	...
	private void UpdateToLastContent()
	{
		...
		if (_positionCtrl.listType == ListPositionCtrl.ListType.Linear)
		{
			if (_contentID == 0 || !nextListBox.isActiveAndEnabled)
			{
				if (!isActiveAndEnabled)
					--_positionCtrl.numOfUpperDisabledBoxes;
