...
public class ListPositionCtrl : MonoBehaviour, IControlEventHandler
{
	...
	private void Start()
	{
		if (listBoxes.Length == 5)
		{
			Rotate(ref listBoxes, (listBoxes.Length - 1) / 2);
			Array.Reverse(listBoxes);
		...
		else
		{
			...
			InitializeBoxDependency();
			_maxNumOfDisabledBoxes = listBoxes.Length / 2;
			foreach (ReverseListBox listBox in listBoxes)
				listBox.Initialize(this);
	...
	static void Rotate<T>(ref T[] array, int shiftCount)
	{
